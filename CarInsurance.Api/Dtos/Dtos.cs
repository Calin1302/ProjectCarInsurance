...
public record CreateClaimDto(string ClaimDate, string? Description, decimal Amount);
public record ClaimDto(long Id, long CarId, string ClaimDate, string? Description, decimal Amount);
public record HistoryItemDto(string Type, string Date, string? Label, string? Details);